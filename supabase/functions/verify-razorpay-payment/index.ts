...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Insert subscription using service role (bypass RLS)
    ...
    if (insErr) {
      ...
      return new Response(JSON.stringify({ error: "Could not save subscription" }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
